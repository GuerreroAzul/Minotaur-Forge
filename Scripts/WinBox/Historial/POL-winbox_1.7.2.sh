...
LOGO="http://imgur.com/IXtdCsI.jpg"
BANNER="http://imgur.com/GwvXo4a.jpg"
MD5_LOGO="b6d33734fed939f0c8a3d15aa24c475c"
MD5_BANNER="978f46d796c71ab265046c8b6af77333"
...
if [ ! -d "$POL_USER_ROOT/configurations/setups/$TITLE" ]; then
  mkdir "$POL_USER_ROOT/configurations/setups/$TITLE"
fi

if [ ! -f "$HOME/.PlayOnLinux/configurations/setups/$TITLE/top" ]; then
  POL_Download_Resource "$LOGO" "$MD5_LOGO" "icons"
  cp "$POL_USER_ROOT/ressources/icons/${$LOGO##*/}" -d "$POL_USER_ROOT/configurations/setups/top"
fi

if [ ! -f "$HOME/.PlayOnLinux/configurations/setups/$TITLE/left" ]; then
  POL_Download_Resource "$BANNER" "$MD5_BANNER" "banners"
  cp "$POL_USER_ROOT/ressources/banners/${$BANNER##*/}" -d "$POL_USER_ROOT/configurations/setups/left"
fi
